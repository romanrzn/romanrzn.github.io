...
main = function() {
  var Car, Path, Point, Table, avg, carsTable, config, dashToCamel, isnot, itemgetter, map, mapCanvas, marketsTable, op, pathsTable, prettyPrintTime, randrange, reduce, resultTable, simulate, sum, tabs, uniqueId, zip, zipWith;
  isnot = function(o) {
    return function(o_) {
      return o_ !== o;
    };
  ...
  Point = (function() {
    function Point(x, y, demand) {
      ...
      mapCanvas.$.drawArc({
        ...
        fillStyle: this.warehouse ? '#37A42C' : "#F65E3B",
        x: x,
        y: y,
    ...
    Point.prototype.resetData = function() {
      return this.data.demand = this.demand;
    };
  ...
  Path = (function() {
    ...
    Path.prototype.removePoint = function(p) {
      this.points = this.points.filter(isnot(p));
    ...
    Path.prototype.removeCar = function(c) {
      this.cars = this.cars.filter(isnot(c));
      return carsTable.update();
    };

    Path.prototype.length = function() {
      return sum(zipWith((function(pp, p) {
        return pp.distanceTo(p);
      }), this.points.slice(0, -1), this.points.slice(1)));
    };
  ...
  simulate = function() {
    var c, cars, d, i, log, p, path, pointsQueue, pp, supply, t, _i, _j, _k, _len, _len1, _len2, _ref, _ref1, _ref2, _ref3, _results;
    _ref = mapCanvas.cars;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      c = _ref[_i];
      c.resetData();
    }
    _ref1 = mapCanvas.points;
    for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
      p = _ref1[_j];
      p.resetData();
    }
    log = function() {
      return console.log.apply(console, ["simulate:"].concat(__slice.call(arguments)));
    };
    _ref2 = mapCanvas.paths;
    for (i in _ref2) {
      path = _ref2[i];
      ...
      while ((p = pointsQueue.shift()) && c) {
        log('point:', p);
        while (p.data.demand > 0 || p.warehouse) {
          d = pp.distanceTo(p);
          ...
          if (p.warehouse) {
            ...
            t = d / config.speed + supply * config.unloadSpeed + config.paperSpeed;
          }
  ...
  mapCanvas = {
        ...
        fn: ((function(_this) {
          return function(ctx) {
            ...
            for (i in _ref) {
              ...
              ctx.textAlign = 'left';
              ctx.fillStyle = '#8F7A66';
              ctx.fillText(p.warehouse ? 'Склад' : 'Магазин ' + i, p.x + 16, p.y + 6);
              if (p.demand) {
                ctx.textAlign = 'right';
                ctx.fillStyle = '#F65E3B';
                _results.push(ctx.fillText(p.demand + ' шт.', p.x + -16, p.y + 6));
              } else {
                _results.push(void 0);
              }
            }
    ...
    removePath: function(p) {
      this.paths = this.paths.filter(isnot(p));
      p.removeLayer();
      pathsTable.update();
      carsTable.update();
      return this.$.drawLayers();
